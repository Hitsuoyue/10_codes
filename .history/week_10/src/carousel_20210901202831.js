...
export default class Carousel extends Component {
    ...
    render() {
        ...
        this.root.addEventListener('pan', event => {
            ...
            console.log('lastIndex', lastIndex, 'nextIndex', nextIndex, `translateX(calc(${position}% - ${Math.abs(x)}px))`, x)
            ...
            if(x > 0) {
                ...
                children[nextIndex].style.transform = `translateX(calc${position}%+${x}px)`;

            } else {
                children[lastIndex].style.transform = `translateX(calc(${position}%-${Math.abs(x)}px))`;
                children[nextIndex].style.transform = `translateX(calc(${position}%-${Math.abs(x)}px))`;
