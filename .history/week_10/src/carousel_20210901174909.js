...
export default class Carousel extends Component {
    ...
    render() {
        ...
        this.root.addEventListener('pan', event => {
            ...
            // x > 0
            // let lastIndex = (current + children.length - 1) % children.length;
            // children[lastIndex].style.transition = 'none';
            // children[lastIndex].style.transform = `translateX(${position - (lastIndex + 1) * 800}px)`;
        
            // x < 0
            // let nextIndex = (current + children.length + 1) % children.length;
            // console.log('nextIndex', nextIndex)
            // children[nextIndex].style.transition = 'none';
            // children[nextIndex].style.transform = `translateX(${position - (nextIndex - 1) * 800}px)`;


            let otherIndex = (current + children.length - current / Math.abs(current)) % children.length;
            children[otherIndex].style.transition = 'none';
            children[otherIndex].style.transform = `translateX(${position - (otherIndex + current / Math.abs(current)) * 800}px)`;

            // let current = position - ((x - x % 500) / 500);
            // console.log('current', current, 'position', position, event);
